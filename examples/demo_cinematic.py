...
from sumo3Dviz import LoaderTools, RenderingTools, SimulationManager, TrajectoryTools
...
def _load_trajectories(loader, configuration):
    ...
    (
        ...
        ego_identifier=configuration["cinematic"]["ego_identifier"],
        simtime_start=configuration["cinematic"]["simtime_start"],
        simtime_end=configuration["cinematic"]["simtime_end"],
        video_fps=configuration["rendering"]["video_fps"],
        show_other_vehicles=configuration["visualization"]["show_other_vehicles"],
    )
    ego_trajectory["veh_id"] = configuration["cinematic"]["ego_identifier"]
...
def _launch_cinematic_mode(
    ...
    df_camera_trajectory_smoothed = (
        trajectory_tools.generate_camera_cinematic_trajectory(
            configuration["cinematic"]["camera_position_trajectory"],
            configuration["cinematic"]["simtime_start"],
            configuration["cinematic"]["simtime_end"],
            configuration["rendering"]["video_fps"],
        )
    )
    # launch simulation manager
...
def _terminate_cinematic_mode(configuration, video_writer):
...
if __name__ == "__main__":
    ...
    _terminate_cinematic_mode(configuration, video_writer)
